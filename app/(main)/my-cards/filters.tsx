...
type CardType = 'all' | 'virtual' | 'physical';

export const filteredCardTypeAtom = atom<CardType>('all');

export function Filters() {
  const [filteredCardType, setFilteredCardType] = useAtom(filteredCardTypeAtom);
  ...
  return (
    <div className='flex flex-col justify-between gap-4 lg:flex-row lg:flex-wrap lg:items-center lg:gap-3'>
      ...
      <SegmentedControl.Root
        value={filteredCardType}
        onValueChange={(v) => setFilteredCardType(v as CardType)}
        className='lg:w-80'
      >
